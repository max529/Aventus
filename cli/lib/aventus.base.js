...
class DateManager {}
class DragAndDrop {    constructor(options) {        this.initialized = false;        this.elementStartTranslate = { x: 0, y: 0 };        this.cursorPositionRelativeToDraggableEl = { x: 0, y: 0 };        const setDefaultValues = () => {            if (!options.classesToAdd) {                options.classesToAdd = 'target;dragging';            }            if (!options.autoCorrectPosition) {                options.autoCorrectPosition = false;            }            if (options.strict === void 0) {                options.strict = true;            }            if (options.blockOutsideTarget === void 0) {                options.blockOutsideTarget = false;            }            if (options.dropCallback === void 0) {                options.dropCallback = () => {                };            }            if (options.shadow === void 0) {                options.shadow = false;            }            else if (options.shadow) {                options.blockOutsideTarget = false;                options.autoCorrectPosition = false;            }            if (options.targets === void 0) {                options.targets = [];            }            if (options.refreshTargets === void 0) {                options.refreshTargets = undefined;            }            if (options.offsetsPercents === void 0) {                options.offsetsPercents = {                    x: 0,                    y: 0                };            }            if (options.cssVars === void 0) {                options.cssVars = null;            }            if (options.useAbsolute === void 0) {                options.useAbsolute = false;            }            if (options.usePercent === void 0) {                options.usePercent = false;            }            if (options.useTargetZoom === void 0) {                options.useTargetZoom = () => {                };            }            if (options.onMove === void 0) {                options.onMove = () => {                };            }            if (options.authorizeDrag === void 0) {                options.authorizeDrag = () => {                    return true;                };            }            if (options.onDragStart === void 0) {                options.onDragStart = () => {                };            }            if (options.onStart === void 0) {                options.onStart = () => {                };            }            if (options.onPressStop === void 0) {                options.onPressStop = () => {                };            }            if (options.onEnd === void 0) {                options.onEnd = () => {                };            }            if (options.getZoom === void 0) {                options.getZoom = () => {                    return 1;                };            }            if (options.afterDrop === void 0) {                options.afterDrop = () => {                };            }            if (options.customSetPosition === void 0) {                options.customSetPosition = undefined;            }            if (options.elementTrigger === void 0) {                options.elementTrigger = options.element;            }            if (options.offsetDrag === void 0) {                options.offsetDrag = undefined;            }            if (options.ignoreDragElement === void 0) {                options.ignoreDragElement = false;            }        };        setDefaultValues();        this.options = options;    }    get strict() {        return this.options.strict;    }    set strict(newValue) {        this.options.strict = newValue;    }    /**     * Check the integrity of the parametersd     */    checkParameter() {        if (this.options.targets.length === 0) {            this.options.blockOutsideTarget = this.options.autoCorrectPosition = false;        }        if (this.options.blockOutsideTarget) {            this.options.autoCorrectPosition = false;            if (this.options.targets.length > 1) {                this.options.targets = [this.options.targets[0]];                console.warn("With blockOutsideTarget set to true, only 1 target can be passed in parameters");            }            this.lastTargetHovered = this.options.targets[0];            this.correctTranslate();        }    }    /**     * Return the position of an event     * @param {PointerEvent} evt - mouseup, mousedown, mousemove, touchstart, touchend, touchmove event     * @returns {Coordinate} The coordinates of the event     */    getEventPosition(evt) {        return {            x: evt.pageX,            y: evt.pageY        };    }    getCurrentPosition(element) {        if (this.options.useAbsolute) {            return this.getAbsolutePosition(element);        }        else if (this.options.cssVars) {            return this.getCssVarPosition(element, this.options.cssVars);        }        return this.getTranslatePosition(element);    }    /**     * Return the position of an element based of his translate style attribute     */    getTranslatePosition(element) {        const res = { x: 0, y: 0 };        if (element.style.transform !== "") {            let temp = element.style.transform.split("translate(")[1];            temp = temp.split(")")[0];            res.x = parseInt(temp.split("px, ")[0]);            res.y = parseInt(temp.split(", ")[1].split("px")[0]);        }        return res;    }    /**     * Return the position of an element based off his absolute position     */    getAbsolutePosition(element) {        return {            x: element.offsetLeft,            y: element.offsetTop        };    }    /**     * Get the position of an element based of his css vars     */    getCssVarPosition(element, cssVars) {        let x = parseInt(element.style.getPropertyValue(cssVars.left.toString()).replace("px", ""));        if (!x) {            x = 0;        }        let y = parseInt(element.style.getPropertyValue(cssVars.top.toString()).replace("px", ""));        if (!y) {            y = 0;        }        return { x, y };    }    initDragAndDrop() {        this.pressManagerInstance = new PressManager({            element: this.options.elementTrigger,            onStart: this.options.onStart,            onDrag: this.startDragndrop,            onStop: this.options.onPressStop,            offsetDrag: this.options.offsetDrag        });        if (this.options.blockOutsideTarget) {            this.correctTranslate();        }    }    destroyPressManager() {        this.pressManagerInstance.destroy();    }    startDragndrop(e, startPosition) {        if (this.options.refreshTargets) {            this.options.targets = this.options.refreshTargets();        }        this.dragStartPosition = { x: startPosition.x, y: startPosition.y };        window.addEventListener("pointerup", this.droppingElement);        window.addEventListener("pointermove", this.draggingElement);    }    /**     * Drag the element to match mouse/touch position     * @param {MouseEvent|TouchEvent} evt - mousemove / touchmove event     */    draggingElement(evt) {        evt.normalize();        if (!this.initialized) {            if (!this.options.authorizeDrag())                return;            this.elementStartTranslate = this.getCurrentPosition(this.options.element);            if (!this.options.ignoreDragElement) {                this.options.element.classList.add("unselectable");                document.body.style.userSelect = "none";            }            if (this.options.shadow) {                const pos = this.options.element.getBoundingClientRect();                this.cursorPositionRelativeToDraggableEl.x = this.dragStartPosition.x - pos.x;                this.cursorPositionRelativeToDraggableEl.y = this.dragStartPosition.y - pos.y;                this.shadowElement = this.options.element.cloneNode(true);                this.shadowElement.style.position = "absolute";                //shadowElement.style.cursor = "grab";                // TODO: Changed from pageContainer to document.body                document.body.appendChild(this.shadowElement);            }            this.options.onDragStart(evt, this, this.shadowElement);            if (this.options.classesToAdd) {                this.options.targets.forEach((element) => {                    element.classList.add(...this.options.classesToAdd);                });            }            this.initialized = true;        }        const position = this.getEventPosition(evt);        let offset = { x: 0, y: 0 };        let newTranslate = { x: 0, y: 0 };        let elementToMove;        if (this.options.shadow) {            newTranslate.x = (position.x - (this.cursorPositionRelativeToDraggableEl.x / this.options.getZoom()));            newTranslate.y = (position.y - (this.cursorPositionRelativeToDraggableEl.y / this.options.getZoom()));        }        else {            offset.x = (position.x - this.dragStartPosition.x) / this.options.getZoom();            offset.y = (position.y - this.dragStartPosition.y) / this.options.getZoom();            newTranslate.x = offset.x + this.elementStartTranslate.x;            newTranslate.y = offset.y + this.elementStartTranslate.y;        }        if (this.options.shadow) {            elementToMove = this.shadowElement;        }        else {            elementToMove = this.options.element;        }        this.setPositionOffset(elementToMove, {            x: newTranslate.x,            y: newTranslate.y        });        if (this.options.targets.length > 0) {            if (this.options.autoCorrectPosition || this.options.blockOutsideTarget) {                const response = this.checkIfInTargets(this.options.element, false);                if (response.success) {                    this.lastTargetHovered = response.value;                }                if (this.options.blockOutsideTarget) {                    this.correctTranslate();                }            }        }    }    /**     * Drop the element to the current position, start position check if needed     * */    droppingElement(e) {        var _a;        if (this.initialized) {            let element = this.options.element;            let target;            let elementCoordinates, targetCoordinates;            if (this.options.shadow) {                element = this.shadowElement;            }            if (this.options.targets.length > 0) {                const response = this.checkIfInTargets(element, this.options.strict);                if (!response.success) {                    if (this.options.autoCorrectPosition) {                        this.correctTranslate();                    }                    else {                        this.setPositionOffset(this.options.element, this.elementStartTranslate);                    }                }                else {                    target = response.value;                    this.options.dropCallback(element, target);                }            }            if (this.options.shadow) {                if (elementCoordinates && targetCoordinates) {                    if (!this.options.useAbsolute) {                        this.shadowElement.style.transform = null;                    }                    this.shadowElement.style.cursor = null;                    this.shadowElement.style.zIndex = "15"; //TODO Need to put a correct z-index, don't know how to do it now                }                else {                    (_a = this.shadowElement.parentNode) === null || _a === void 0 ? void 0 : _a.removeChild(this.shadowElement);                }            }            if (this.options.classesToAdd) {                this.options.targets.forEach((element) => {                    element.classList.remove(...this.options.classesToAdd);                });            }            this.options.afterDrop(this.shadowElement, target);            this.options.element.classList.remove("unselectable");            document.body.style.userSelect = null;            this.options.onEnd(e, this);            this.initialized = false;        }        window.removeEventListener("pointerup", this.droppingElement);        window.removeEventListener("pointermove", this.draggingElement);    }    /**     * Check if the element is in on of the targets of the targets list     * @param {boolean} strict - If true, return true if the element touch the target (not contained into)     * @returns {Reponse} The operation response, return the node the element was the closest to if strict = false     */    checkIfInTargets(element = this.options.element, strict = true) {        let result = {            success: false,        };        for (let i = 0; i < this.options.targets.length; i++) {            let response = this.checkIfInTarget(element, this.options.targets[i], this.options.strict);            if (response.success &&                strict) {                return response;            }            else if (response.success &&                !strict) {                result.success = true;                result.value = response.value;                break;            }        }        return result;    }    /**     * Check if element is in target     * @param {HTMLElement} element - The element to check     * @param {HTMLElement} target - The target to check     * @param {boolean} strict - If true, we check if the element touch the target (not contained)     */    checkIfInTarget(element, target, strict = true) {        const response = {            success: false,        };        const elementCoordinate = element.getBoundingClientRect();        const targetCoordinate = target.getBoundingClientRect();        const zoom = this.options.useTargetZoom();        const shiftX = (zoom === null || zoom === void 0 ? void 0 : zoom.x()) || 0;        const shiftY = (zoom === null || zoom === void 0 ? void 0 : zoom.y()) || 0;        const scale = (zoom === null || zoom === void 0 ? void 0 : zoom.scale()) || 1;        targetCoordinate.x += shiftX;        targetCoordinate.y += shiftY;        targetCoordinate.width *= scale;        targetCoordinate.height *= scale;        if (strict) {            if ((elementCoordinate.x >= targetCoordinate.x && elementCoordinate.x + elementCoordinate.width <= targetCoordinate.x + targetCoordinate.width) &&                (elementCoordinate.y >= targetCoordinate.y && elementCoordinate.y + elementCoordinate.height <= targetCoordinate.y + targetCoordinate.height)) {                response.success = true;                response.value = target;                response.elementCoordinate = elementCoordinate;                response.targetCoordinate = targetCoordinate;            }        }        else {            let elementLeft = elementCoordinate.x;            let elementRight = elementCoordinate.x + elementCoordinate.width;            let elementTop = elementCoordinate.y;            let elementBottom = elementCoordinate.y + elementCoordinate.height;            let targetLeft = targetCoordinate.x;            let targetRight = targetCoordinate.x + targetCoordinate.width;            let targetTop = targetCoordinate.y;            let targetBottom = targetCoordinate.y + targetCoordinate.height;            if (!(elementRight < targetLeft ||                elementLeft > targetRight ||                elementBottom < targetTop ||                elementTop > targetBottom)) {                response.success = true;                response.value = target;            }        }        return response;    }    /**     * Correct the margin of the draggable element to match the target dimension     * */    correctTranslate() {        if (this.lastTargetHovered) {            const translate = this.getTranslatePosition(this.options.element);            if (translate.x + this.options.element.offsetWidth > this.lastTargetHovered.offsetWidth + this.lastTargetHovered.offsetLeft) {                this.setTransformPositionOffset(this.options.element, {                    x: this.lastTargetHovered.offsetWidth + this.lastTargetHovered.offsetLeft - this.options.element.offsetWidth                });            }            else if (translate.x < this.lastTargetHovered.offsetLeft) {                this.setTransformPositionOffset(this.options.element, {                    x: this.lastTargetHovered.offsetLeft                });            }            if (translate.y + this.options.element.offsetHeight > this.lastTargetHovered.offsetHeight + this.lastTargetHovered.offsetTop) {                this.setTransformPositionOffset(this.options.element, {                    y: this.lastTargetHovered.offsetHeight + this.lastTargetHovered.offsetTop - this.options.element.offsetHeight                });            }            else if (translate.y < this.lastTargetHovered.offsetTop) {                this.setTransformPositionOffset(this.options.element, {                    y: this.lastTargetHovered.offsetTop                });            }        }        else {            this.setTransformPositionOffset(this.options.element, this.elementStartTranslate);        }    }    /**     * Set element margin     * @param {HTMLElement} element - The element to set coordinate     * @param {Coordinates} coordinate - The X and Y value     */    setTransformPositionOffset(element, coordinate) {        const currentTransform = this.getTranslatePosition(element);        let transformX = coordinate.x !== undefined ? coordinate.x : currentTransform.x;        let transformY = coordinate.y !== undefined ? coordinate.y : currentTransform.y;        if (this.options.cssVars != null) {            element.style.setProperty(this.options.cssVars.top, transformY + "px");            element.style.setProperty(this.options.cssVars.left, transformX + "px");        }        else {            if (!this.options.ignoreDragElement) {                element.style.transform = `translate(${transformX}px, ${transformY}px)`;            }        }        this.options.onMove(transformY + "px", transformX + "px");    }    /**     * Set the offset to apply to an element     * @param {HTMLElement} element - The element to set the offset of     * @param {Object} coordinate - The x and y value     */    setAbsolutePositionOffset(element, coordinate) {        const currentTransform = this.getAbsolutePosition(element);        if (!coordinate.x) {            coordinate.x = currentTransform.x;        }        if (!coordinate.y) {            coordinate.y = currentTransform.y;        }        if (this.options.customSetPosition) {            this.options.customSetPosition(element, coordinate);        }        else if (this.options.usePercent) {            let elementParent = element.offsetParent;            const percentLeft = (coordinate.x / elementParent.offsetWidth) * 100 + this.options.offsetsPercents.x;            const percentTop = (coordinate.y / elementParent.offsetHeight) * 100 + this.options.offsetsPercents.y;            if (this.options.cssVars != null) {                element.style.setProperty(this.options.cssVars.top, percentTop + '%');                element.style.setProperty(this.options.cssVars.left, percentLeft + '%');            }            else {                element.style.top = percentTop + '%';                element.style.left = percentLeft + '%';            }            this.options.onMove(percentTop + '%', percentLeft + '%');        }        else {            if (this.options.cssVars != null) {                element.style.setProperty(this.options.cssVars.top, coordinate.y + 'px');                element.style.setProperty(this.options.cssVars.left, coordinate.x + 'px');            }            else {                element.style.top = coordinate.y + 'px';                element.style.left = coordinate.x + 'px';            }            this.options.onMove(coordinate.y + 'px', coordinate.x + 'px');        }    }    setPositionOffset(element, coordinate) {        element.dispatchEvent(new CustomEvent("move"));        if (this.options.useAbsolute) {            this.setAbsolutePositionOffset(element, coordinate);        }        else {            this.setTransformPositionOffset(element, coordinate);        }    }    /**     * Set the blockOutsideTarget parameter     * @param {boolean} newBlockOutsideTarget - The new parameter value     */    setBlockOutsideTarget(newValue) {        this.options.blockOutsideTarget = newValue;        this.checkParameter();    }    /**     * Set the autoCorrectPosition parameter     * @param {boolean} newAutoCorrectPosition - The new parameter value     */    setAutoCorrectPosition(newAutoCorrectPosition) {        this.options.autoCorrectPosition = newAutoCorrectPosition;        this.checkParameter();    }    /**     * Set the targets of the drag and drop     * @param {HTMLElement[]} newTargets - The new drag and drop targets     */    setTargets(newTargets) {        this.options.targets = newTargets;        this.checkParameter();    }    /**     * Set the after drop event function     * @param {Function} newFunction - The function to trigger when element is dropped     */    setAfterDrop(newFunction) {        this.options.afterDrop = newFunction;    }    /**     * Return if the element is in one of the targets     * @param {boolean} strict - If true, check if the element is entierely in the target. Default to true     * @returns {Response} Return response.sucess false if outside, true otherwise. Response.value contains the target in which the element is, if any.     */    isInTarget(strict = true) {        return this.checkIfInTargets(null, strict);    }    /**     * Get the current target     * @returns {Element} - The current target     */    getCurrentTarget() {        return this.options.shadow ? this.shadowElement : this.options.element;    }    /**     * Set the offsets in percentage     * @param {number} x - The X coordinate     * @param {number} y - The Y coordinate     */    setOffsetsPercents(coordinate) {        this.options.offsetsPercents = coordinate;    }    /**     * Destroy the drag and drop events     */    destroy() {        this.destroyPressManager();    }}
                    ...
                    if (prop == 'push') {
                        ...
                        result = (index, nbRemove, ...insert) => {
                            ...
                            for (let i = 0; i < res.length; i++) {
                                trigger('DELETED', path + "[" + (index + i) + "]", res[i]);
                            }
...
class Device {    static get os() {        if (navigator.userAgent.indexOf("raspberry-os") !== -1) {            return DeviceType.RaspberryOS;        }        else if ([            "iPad Simulator",            "iPhone Simulator",            "iPod Simulator",            "iPad",            "iPhone",            "iPod"        ].includes(navigator.platform) || (navigator.userAgent.includes("Mac") && "ontouchend" in document)) {            return DeviceType.IpadOS;        }        return DeviceType.Other;    }}
var DeviceType;(function (DeviceType) {    DeviceType[DeviceType["RaspberryOS"] = 0] = "RaspberryOS";    DeviceType[DeviceType["IpadOS"] = 1] = "IpadOS";    DeviceType[DeviceType["Other"] = 2] = "Other";})(DeviceType || (DeviceType = {}));
...
var MutableAction;(function (MutableAction) {    MutableAction[MutableAction["CREATED"] = 0] = "CREATED";    MutableAction[MutableAction["UPDATED"] = 1] = "UPDATED";    MutableAction[MutableAction["DELETED"] = 2] = "DELETED";})(MutableAction || (MutableAction = {}));
