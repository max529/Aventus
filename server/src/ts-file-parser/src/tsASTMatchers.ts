...
export module Matching {
    ...
    export class VariableDeclarationMatcher extends BasicMatcher implements TypedMatcher<ts.VariableDeclaration> {
        match(node: ts.VariableDeclaration): any {
            if (this.left.doMatch(node.name) && node.initializer && this.right.doMatch(node.initializer)) {
                return this.tr(node);
            }
    ...
    export function visit<T>(n: Node, cb: NodeCallback<T>): T | undefined {
        var r0 = cb(n);
        if (r0) {
            if (r0 === SKIP) {
                return undefined;
        ...
        var r: T | undefined = ts.forEachChild<T>(n, x => {
            var r = visit(x, cb);
            if (r) {
                return r;
            }
    ...
    export class PathNode {
        name: string;
        arguments: ReadonlyArray<Expression> | null = null;
        _callExpression: ts.CallExpression | null = null;
    ...
    export function memberFromExp(objMatcher: string, tr: Transformer<Expression, any> = x => true): TypedMatcher<any> | null {
        var array: string[] = objMatcher.split(".");
        var result: TypedMatcher<any> | null = null;
    ...
    export class CallBaseMatcher implements TypedMatcher<Expression> {
        doMatch(node: Expression): CallPath | null {
            var original = node;
            if (node.kind === ts.SyntaxKind.CallExpression) {
                var call = (<CallExpression>node);
                var res: CallPath | null = this.doMatch(call.expression);
                if (res) {
                ...
                var v: CallPath | null = this.doMatch(me.expression);
                if (v) {
    ...
    export function exprStmt(eM: TypedMatcher<Expression>,
        tr: Transformer<ts.Expression, any> = x => true): TypedMatcher<ts.ExpressionStatement> {
