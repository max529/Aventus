...
class AvTodoList extends WebComponent {
	...
	__prepareForLoop() {
		super.__prepareForLoop(); this.__loopTemplate['avtodolist_1'] = `    <av-todo-item _id="avtodolist_0"></av-todo-item>`;
...
<div class="name" _id="avtodoitem_1"></div>`
	...
	__registerOnChange() {
		super.__registerOnChange(); this.__onChangeFct['item'] = []
		this.__onChangeFct['item'].push((path) => {
			if ("item.state".startsWith(path)) {
...
<av-for item="light" in="data" index="i" _id="avcomplextest_4"></av-for>`
	...
	__prepareForLoop() {
		super.__prepareForLoop(); this.__loopTemplate['avcomplextest_5'] = `        <div _id="avcomplextest_3"></div>    `;
	...
	__registerOnChange() {
		super.__registerOnChange(); this.__onChangeFct['testvariable'] = []
		this.__onChangeFct['testvariable'].push((path) => {
			if ("testvariable".startsWith(path)) {
	...
	__defaultValue() {
		super.__defaultValue(); if (!this.hasAttribute('testvariable')) { this['testvariable'] = 'test'; }
	}
	__upgradeAttributes() {
		super.__upgradeAttributes(); this.__upgradeProperty('testvariable');
	}
	postCreation() {
		let i = 0;
